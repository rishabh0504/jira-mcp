...
// Define the tool
export const fetchJiraTicketsTool = new DynamicStructuredTool({
  ...
  func: async (input: any) => {
    ...
    const queryPrompt = JSON.parse(input);
    console.log("===============================>>>", queryPrompt);
    const { project_key, query } = queryPrompt ?? {};
    // return `Hello, ${queryPrompt?.name}`;
    ...
    try {
      ...
      // For Jira Server, use Basic Auth with username/password approach
